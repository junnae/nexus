...
5. On the child's tap-to-reveal gesture: play `audio_word_path` aloud (the read-aloud "solution") and show a small replay-sound control for the rest of the round
...
7. Emit `onTileDrop` event with tile ID and drop position
...
4. On tap-to-reveal: resume the `AudioContext` (first real user gesture) and play `audio_word_path` — the read-aloud "solution"; a small replay-sound control (positioned just below the answer slots, from the measured answer-area rect) repeats this on demand for the rest of the round
